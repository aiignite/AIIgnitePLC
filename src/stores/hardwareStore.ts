...
const DEFAULT_MODULES: HardwareModule[] = [
  ...
  },
  {
    slot: 2,
    name: 'CPU 1511-1 PN',
    articleNumber: '6ES7 511-1AK02-0AB0',
    firmware: 'V2.9',
    type: 'cpu',
    ip: '192.168.0.1',
  ...
  {
    slot: 4,
    name: 'DQ 16x24VDC/0.5A',
    articleNumber: '6ES7 522-1BH01-0AB0',
    firmware: 'V1.1',
    type: 'io',
    ioStart: 0,
    ioLength: 2,
  ...
  {
    slot: 5,
    name: 'AI 8xU/I/RTD/TC',
    articleNumber: '6ES7 531-7KF00-0AB0',
    firmware: 'V1.0',
    type: 'io',
    ioStart: 64,
    ioLength: 16,
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.length === 0) {
          ...
          // If empty, seed defaults to server and show in UI
          set({ modules: DEFAULT_MODULES, isLoading: false });

          // Persist defaults in background
