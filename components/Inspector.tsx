...
export const Inspector: React.FC<InspectorProps> = ({
  ...
  const renderContent = () => {
    ...
    if (mainTab === 'diagnostics') {
      return (
        <div className="p-4">
          <h5 className="font-bold text-slate-800 text-sm mb-4 pb-1 border-b border-slate-200">
            诊断信息 (Diagnostics)
          </h5>
          <table className="w-full text-xs text-left border-collapse border border-slate-300">
            ...
            <tbody>
              <tr>
                <td className="p-2 border border-slate-300 text-green-600 flex items-center gap-1">
      ...
      return (
        <div className="p-4">
          ...
          <div className="grid grid-cols-[150px_1fr] gap-4 text-xs">
            ...
            <input
              type="text"
              className="border border-slate-300 p-1 w-full max-w-md"
            ...
            <textarea
              className="border border-slate-300 p-1 w-full max-w-md h-20"
              ...
              <>
                <div className="bg-yellow-50 border border-yellow-200 p-3 rounded">
                  ...
                  <input
                    ...
                    onChange={e => {
                      const current = selectedElement.parameters || [];
                      const updated = current.some(p => p.name === 'PT')
                        ? current.map(p => (p.name === 'PT' ? { ...p, value: e.target.value } : p))
                        : [...current, { name: 'PT', value: e.target.value }];
                      onUpdateElement(selectedElement.id, 'parameters', updated);
                    }}
                  />
      ...
      return (
        <div className="p-4 max-w-lg">
          ...
          <div className="grid grid-cols-[100px_1fr] gap-y-3 gap-x-4 text-xs items-center">
            ...
            <div className="font-mono font-bold bg-slate-100 px-2 py-1 border border-slate-200">
              {selectedElement.tag}
            </div>
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-[100px_1fr] items-center gap-2">
              ...
              <div className="relative">
                ...
                />
                <span className="absolute right-2 top-1 text-[10px] text-orange-500 font-bold">
                  %
                </span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-[100px_1fr] items-center gap-2">
              <label className="text-xs text-slate-500 text-right">实时值:</label>
              <div
                className={`text-xs font-mono ${runtimeValue?.quality === 'bad' ? 'text-red-600' : 'text-green-600'}`}
              >
                {runtimeValue?.value !== undefined ? String(runtimeValue.value) : '--'}
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              <input
                ...
                defaultChecked
              />
              <label htmlFor="chk1" className="text-xs text-slate-700">
                在 HMI / OPC UA 中可见
              </label>
            </div>
            <div className="flex items-center gap-2">
              <input
                ...
                disabled
              />
              <label htmlFor="chk2" className="text-xs text-slate-400">
                保持性 (需要全局设置)
              </label>
            </div>
          </div>
