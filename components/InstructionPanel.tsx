...
import { Instruction, ChatMessage } from '../types';
...
const INSTRUCTIONS: Instruction[] = [
  { id: 'i1', name: '常开触点', category: 'BitLogic', type: 'contactNO', description: 'Normally Open' },
  { id: 'i2', name: '常闭触点', category: 'BitLogic', type: 'contactNC', description: 'Normally Closed' },
...
export const InstructionPanel: React.FC<InstructionPanelProps> = ({ onAddInstruction, chatMessages, onSendMessage }) => {
  const [activeTab, setActiveTab] = useState<'instructions' | 'ai'>('instructions');
  // Changed to Set for multiple expansions
  const [expandedCategories, setExpandedCategories] = useState<Set<string>>(new Set(['BitLogic', 'Timer']));
        ...
        <div className="flex-1 overflow-y-auto bg-white min-w-0">
           <div className="p-2 space-y-1">
             ...
             {['BitLogic', 'Timer', 'Counter', 'Comparator', 'Math', 'Move'].map(cat => {
               const catLabel = cat === 'BitLogic' ? '位逻辑运算 (Bit Logic)' : 
                                cat === 'Timer' ? '定时器操作 (Timers)' : 
                                cat === 'Counter' ? '计数器操作 (Counters)' :
                                cat === 'Math' ? '数学函数 (Math)' : cat;
               ...
               return (
                 <div key={cat} className="border border-slate-200 rounded overflow-hidden">
                   ...
                   >
                     <span className="material-symbols-outlined text-[16px] text-slate-500 transition-transform duration-200" style={{ transform: isOpen ? 'rotate(90deg)' : 'rotate(0deg)' }}>
                       chevron_right
                     </span>
                         ...
                         {INSTRUCTIONS.filter(i => i.category === cat).map(inst => (
                           ...
                           >
                             <span className="material-symbols-outlined text-[24px] text-primary mb-1">
                               {inst.type === 'contactNO' ? 'check_box_outline_blank' : 
                                inst.type === 'contactNC' ? 'disabled_by_default' : 
                                inst.type === 'coil' ? 'code' : 'crop_square'}
                             </span>
                             <span className="text-xs font-medium text-slate-800 leading-tight w-full truncate">{inst.name}</span>
                             <span className="text-[10px] text-slate-400 font-mono">{inst.description}</span>
                           </div>
                         ))}
                       </div>
                       
                       {INSTRUCTIONS.filter(i => i.category === cat).length === 0 && (
                         <div className="p-2 text-[10px] text-slate-400 italic text-center">暂无指令</div>
                       )}
                     </div>
                   )}
                 </div>
