...
const INITIAL_CONNECTIONS: Connection[] = [
  { id: 'c1', fromBlockId: 'block_start', fromPortId: 'out1', toBlockId: 'block_and', toPortId: 'in1' },
  { id: 'c2', fromBlockId: 'block_stop', fromPortId: 'out1', toBlockId: 'block_and', toPortId: 'in2' },
  { id: 'c3', fromBlockId: 'block_and', fromPortId: 'out1', toBlockId: 'block_pid', toPortId: 'in_en' }
];
...
export const GraphicEditor: React.FC = () => {
  ...
  const [pendingConnection, setPendingConnection] = useState<{ blockId: string; portId: string; type: 'input' | 'output'; mousePos: Point } | null>(null);
  ...
  const handleDelete = useCallback((e?: React.MouseEvent) => {
    ...
    if (selectedBlockId) {
      ...
      setConnections(prev => prev.filter(c => c.fromBlockId !== selectedBlockId && c.toBlockId !== selectedBlockId));
      setSelectedBlockId(null);
  ...
  const handlePortMouseUp = (e: React.MouseEvent, targetBlockId: string, targetPortId: string, targetType: 'input' | 'output') => {
    e.stopPropagation();
    if (pendingConnection) {
      ...
      if (sourceType !== targetType && sourceBlockId !== targetBlockId) {
        ...
        const isOccupied = connections.some(c => c.toBlockId === toBlockId && c.toPortId === toPortId);
        if (!isOccupied) {
          setConnections(prev => [...prev, { id: `c_${Date.now()}`, fromBlockId, fromPortId, toBlockId, toPortId }]);
        }
      }
    }
    setPendingConnection(null);
    ...
    >
      ...
      <div className="h-10 bg-white border-b border-slate-300 flex items-center px-4 gap-4 shrink-0 z-30 shadow-sm">
        <div className="flex items-center gap-2 border-r border-slate-200 pr-4">
          <button className="p-1 hover:bg-slate-100 rounded text-slate-500"><span className="material-symbols-outlined text-[18px]">undo</span></button>
          <button className="p-1 hover:bg-slate-100 rounded text-slate-500"><span className="material-symbols-outlined text-[18px]">redo</span></button>
        </div>
           ...
           <button 
             onClick={() => {
               const newBlock: GraphicBlock = {
                 ...
                 type: 'logic',
                 label: 'AND',
                 pos: { x: 100 + (canvasRef.current?.scrollLeft || 0), y: 100 + (canvasRef.current?.scrollTop || 0) },
        ...
        <div className="flex items-center gap-2">
           {(selectedBlockId || selectedConnectionId) && (
             <button onClick={(e) => handleDelete(e)} className="flex items-center gap-1 px-3 py-1 bg-red-50 text-red-600 border border-red-200 rounded text-xs font-bold hover:bg-red-100">
           ...
           <div className="flex items-center gap-1 bg-slate-100 px-2 py-1 rounded border border-slate-200">
              <button className="size-5 flex items-center justify-center hover:bg-white rounded"><span className="material-symbols-outlined text-[14px]">remove</span></button>
              <span className="text-[10px] font-bold text-slate-600 px-1">100%</span>
              <button className="size-5 flex items-center justify-center hover:bg-white rounded"><span className="material-symbols-outlined text-[14px]">add</span></button>
           </div>
        </div>
      </div>
      ...
      >
        ...
        <svg className="absolute inset-0 w-[2000px] h-[2000px] z-10 overflow-visible pointer-events-none">
          <defs>
            <marker id="arrowhead" markerWidth="6" markerHeight="4" refX="5" refY="2" orientation="auto"><polygon points="0 0, 6 2, 0 4" fill="#137fec"/></marker>
            <marker id="arrowhead-sel" markerWidth="6" markerHeight="4" refX="5" refY="2" orientation="auto"><polygon points="0 0, 6 2, 0 4" fill="#ef4444"/></marker>
          </defs>
          ...
          {connections.map(conn => {
            ...
            return (
              <g key={conn.id} className="pointer-events-auto cursor-pointer group" onClick={(e) => handleSelectConnection(e, conn.id)}>
                <path d={d} stroke="transparent" strokeWidth="16" fill="none" />
                <path d={d} stroke={isSelected ? '#ef4444' : '#137fec'} strokeWidth={isSelected ? '4' : '2.5'} fill="none" markerEnd={isSelected ? 'url(#arrowhead-sel)' : 'url(#arrowhead)'} className="transition-all" />
              </g>
            );
          })}
          ...
          {pendingConnection && (() => {
            ...
            return <path d={`M ${sX} ${sY} C ${sX+cO} ${sY}, ${eX-cO} ${eY}, ${eX} ${eY}`} stroke="#137fec" strokeWidth="2" strokeDasharray="4 2" fill="none" opacity="0.6" />;
          })()}
        </svg>
        ...
        <div className="relative z-20 w-[2000px] h-[2000px] pointer-events-none">
          {blocks.map(block => {
            ...
            return (
              ...
              >
                {/* Header */}
                <div className={`flex items-center justify-between px-2 py-0.5 border-b-2 ${block.type === 'function' ? 'bg-primary text-white border-primary' : 'bg-slate-100 border-slate-800'} ${block.type === 'function' ? 'h-[24px]' : 'h-[20px]'}`}>
                  ...
                  <div className="flex gap-1 items-center">
                    ...
                    <span className="material-symbols-outlined text-[14px] cursor-pointer hover:opacity-80">settings</span>
                  </div>
                </div>
                ...
                <div className="flex-1 flex relative p-1 pb-2">
                  <div className="flex flex-col gap-2 min-h-[40px] flex-1">
                    {block.inputs.map(port => (
                      <div key={port.id} className="flex items-center relative h-6">
                        ...
                        ></div>
                        <span className="text-[10px] font-bold text-slate-600 pl-1.5">{port.name}</span>
                      </div>
                    ))}
                  </div>
                  <div className="flex flex-col gap-2 min-h-[40px] flex-1 items-end">
                    {block.outputs.map(port => (
                      <div key={port.id} className="flex items-center relative h-6">
                        <span className="text-[10px] font-bold text-slate-600 pr-1.5">{port.name}</span>
                        <div 
      ...
      {/* Resizable Console */}
      <div style={{ height: consoleHeight }} className="bg-slate-900 border-t border-slate-800 flex flex-col shrink-0 z-40 relative">
         <div 
           ...
           onMouseDown={(e) => { e.preventDefault(); setIsResizingConsole(true); }}
         ></div>
         <div className="flex bg-slate-800/50 border-b border-white/10 shrink-0">
            <button className="px-4 py-1.5 text-[10px] font-bold text-blue-400 border-b-2 border-blue-400 bg-white/5">Compiler Output</button>
            <button className="px-4 py-1.5 text-[10px] font-bold text-slate-500 hover:text-slate-300">Errors (0)</button>
            <button className="px-4 py-1.5 text-[10px] font-bold text-slate-500 hover:text-slate-300">Warnings (1)</button>
         </div>
         <div className="flex-1 p-2 font-mono text-[10px] overflow-y-auto text-slate-300">
            ...
            <p className="text-blue-400">Blocks: {blocks.length}, Nets: {connections.length}.</p>
            {connections.length < blocks.length && <p className="text-yellow-400">Warning: Open inputs detected.</p>}
