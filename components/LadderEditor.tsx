...
import { LadderElement, LadderRung, Network } from '../types';

interface LadderEditorProps {
  networks: Network[];
  selectedNetworkId?: string | null;
  onNetworkSelect?: (networkId: string) => void;
  onNetworkRename?: (networkId: string) => void;
  selectedElementId: string | null;
  onElementSelect: (element: LadderElement) => void;
  onDeleteElement: (elementId: string) => void;
  runtimeValues?: Map<string, RuntimeValue>;
  diagnostics?: Array<{ elementId?: string; severity?: string }>;
}
...
const ElementRenderer: React.FC<{
  ...
  runtimeValues?: Map<string, RuntimeValue>;
  hasError?: boolean;
}> = ({ element, isSelected, onClick, onDelete, runtimeValues, hasError }) => {
  const runtimeValue = runtimeValues?.get(element.address);
  const isActive = runtimeValue?.value === true || runtimeValue?.value === 1;
  const wireColor = isActive ? 'bg-green-500' : 'bg-black';

  // TIA Portal Style: Address (top, often purple/grey), Tag (middle, bold), Symbol (bottom)
  return (
    <div
      className={`flex flex-col items-center relative group/element cursor-pointer select-none mx-1 min-w-[60px] ${hasError ? 'ring-2 ring-red-500 rounded' : ''}`}
      onClick={e => {
        e.stopPropagation();
        onClick();
      }}
    >
      ...
      <div className="flex flex-col items-center mb-1">
        ...
        <span
          className={`text-[10px] font-mono leading-tight ${isSelected ? 'text-primary' : isActive ? 'text-green-600' : 'text-purple-700'}`}
        >
          {element.address}
        </span>
        {/* Tag Name: bold, black */}
        <span
          className={`text-[11px] font-bold leading-tight px-1 rounded ${isSelected ? 'bg-primary text-white' : isActive ? 'text-green-700' : 'text-black'}`}
        >
          {`"${element.tag}"`}
        </span>
      </div>
      ...
      <div className="relative">
        ...
        {element.type === 'contactNO' && (
          <div className="w-10 h-8 flex items-center justify-center relative">
            <div
              className={`absolute left-0 top-1/2 w-3 h-0.5 ${wireColor} -translate-y-1/2`}
            ></div>
            <div
              className={`absolute right-0 top-1/2 w-3 h-0.5 ${wireColor} -translate-y-1/2`}
            ></div>
            <div className={`w-0.5 h-6 ${wireColor} mx-1`}></div>
            <div className={`w-0.5 h-6 ${wireColor} mx-1`}></div>
          </div>
        )}
        ...
        {element.type === 'contactNC' && (
          <div className="w-10 h-8 flex items-center justify-center relative">
            ...
            <div className={`w-0.5 h-6 ${wireColor} mx-1`}></div>
            <div className={`w-0.5 h-6 ${wireColor} mx-1`}></div>
            <div className={`absolute w-6 h-0.5 ${wireColor} rotate-45`}></div>
          </div>
        )}
        ...
        {element.type === 'coil' && (
          <div className="w-12 h-8 flex items-center justify-center relative">
            <div
              className={`absolute left-0 top-1/2 w-2 h-0.5 ${wireColor} -translate-y-1/2`}
            ></div>
            <div
              className={`absolute right-0 top-1/2 w-2 h-0.5 ${wireColor} -translate-y-1/2`}
            ></div>
            {/* Parentheses shape */}
            <svg
              width="32"
            ...
            >
              <path
                d="M8 2C4 6 4 18 8 22"
                stroke={isActive ? '#22c55e' : 'black'}
                strokeWidth="2"
                fill="none"
              />
              <path
                d="M24 2C28 6 28 18 24 22"
                stroke={isActive ? '#22c55e' : 'black'}
                strokeWidth="2"
                fill="none"
              />
        ...
        {element.type === 'box_timer' && (
          <div
            className={`border-2 border-slate-800 bg-white w-32 min-h-[100px] relative flex flex-col z-10 ${isSelected ? 'shadow-[0_0_0_2px_rgba(19,127,236,0.5)]' : ''}`}
          >
            {/* Header */}
...
const RungRenderer: React.FC<{
  ...
  runtimeValues?: Map<string, RuntimeValue>;
  errorIds?: Set<string>;
}> = ({ rung, selectedElementId, onElementSelect, onDeleteElement, runtimeValues, errorIds }) => {
  return (
      ...
      <div className="ml-4 flex items-center">
        ...
        {rung.elements.map((el, _index) => (
          <React.Fragment key={el.id}>
            ...
            {el.type === 'box_timer' && (
              <div
                className="absolute w-8 h-0.5 bg-black -translate-y-9 translate-x-[128px]"
                style={{ zIndex: -1 }}
              ></div>
            )}
          </React.Fragment>
        ...
        <>
          ...
          <div className="absolute top-[108px] left-[60px]">
            <ElementRenderer
              ...
              runtimeValues={runtimeValues}
              hasError={errorIds?.has(rung.branchElement.id)}
            />
          </div>
...
export const LadderEditor: React.FC<LadderEditorProps> = ({
  ...
  const errorIds = new Set(
    (diagnostics || [])
      .filter(d => d.severity === 'error' && d.elementId)
  ...
  return (
    <div className="flex-1 overflow-y-auto p-2 space-y-2 bg-slate-100">
      {networks.map(net => (
        <div
          key={net.id}
          className="bg-white border border-slate-300 shadow-sm overflow-hidden group/network"
        >
          {/* Network Header */}
          <div
            className={`bg-slate-100 border-b border-slate-300 px-3 py-1 flex justify-between items-start cursor-default select-none h-8 ${selectedNetworkId === net.id ? 'ring-1 ring-primary' : ''}`}
            onClick={() => onNetworkSelect?.(net.id)}
            onDoubleClick={() => onNetworkRename?.(net.id)}
          >
