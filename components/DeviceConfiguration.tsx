...
export const DeviceConfiguration: React.FC = () => {
  ...
  const handleUpdateModule = (
    ...
    if (module) {
      ...
      updateModule(currentProjectId, slot, updated);
    }
  };
          ...
          <div className="flex-1 overflow-y-auto p-2">
            ...
            <div className="space-y-1">
              {CATALOG.map(item => (
                ...
                >
                  <div className="text-xs font-bold text-slate-700 group-hover:text-primary">
                    {item.name}
                  </div>
    ...
    <div className="flex-1 p-8 bg-white relative overflow-auto h-full">
      ...
      <div className="flex items-center gap-20 mt-10 ml-10">
        ...
        <div className="relative w-32 h-40 border-2 border-primary bg-slate-50 shadow-md flex flex-col group hover:shadow-lg transition-shadow">
          ...
          {/* Port */}
          <div className="absolute -right-3 bottom-4 size-6 bg-green-500 border-2 border-white rounded-sm flex items-center justify-center text-[8px] text-white font-bold z-10 cursor-pointer hover:scale-110 transition-transform">
            P1
          </div>
        </div>
        ...
        <div className="relative w-32 h-40 border border-slate-300 bg-slate-50 shadow-sm flex flex-col group hover:shadow-lg transition-shadow">
          ...
          <div className="absolute -left-3 bottom-4 size-6 bg-green-500 border-2 border-white rounded-sm flex items-center justify-center text-[8px] text-white font-bold z-10">
            P1
          </div>
          <div className="absolute -right-3 bottom-4 size-6 bg-green-500 border-2 border-white rounded-sm flex items-center justify-center text-[8px] text-white font-bold z-10">
            P2
          </div>
        </div>
  ...
  const renderTopologyView = () => (
    <div className="flex-1 p-8 bg-slate-50 relative overflow-auto h-full">
      ...
      <div className="mt-4 text-center text-xs text-slate-500">
        此视图显示物理端口互连 (Physical Port Interconnection)
      </div>
    </div>
  );
    ...
    switch (activeTab) {
      case 'profinet':
        if (module.type !== 'cpu' && module.type !== 'comm') {
          return (
            <div className="p-4 text-xs text-slate-400 italic">此模块没有 PROFINET 接口。</div>
          );
        }
        return (
          <div className="p-4 space-y-6 max-w-2xl">
            <h5 className="font-bold text-slate-800 text-xs mb-2 border-b border-slate-200 pb-1">
              以太网地址 (Ethernet addresses)
            </h5>
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-[120px_1fr] gap-2 text-xs items-center">
                ...
                <div className="flex gap-2 items-center">
                  ...
                  />
                  <button className="px-2 py-0.5 bg-slate-100 border border-slate-300 rounded hover:bg-slate-200">
                    自动生成
                  </button>
                </div>
              </div>
        ...
        return (
          <div className="p-4 space-y-6 max-w-2xl">
            ...
            <div className="bg-yellow-50 p-2 text-xs text-yellow-800 border border-yellow-200 rounded">
              过程映像 (Process Image):{' '}
              <span className="font-bold">自动更新 (Automatic update)</span>
            </div>
          </div>
        );
              ...
              <table className="w-full text-xs text-left">
                ...
                <tbody>
                  ...
                  {module.type === 'io' && (
                    <tr>
                      <td className="p-2 border-r border-slate-200 font-mono">
                        Local~{module.name.replace(/\s+/g, '_')}_Port
                      </td>
  ...
  const renderInspector = () => {
    ...
    return (
      <div
        style={{ height: inspectorHeight }}
        className="bg-white border-t border-slate-300 flex flex-col shrink-0 z-20 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)] transition-all relative"
      >
        {/* Resizer Handle */}
        <div
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <div className="w-48 border-r border-slate-200 bg-slate-50 p-2 overflow-y-auto shrink-0">
            <div className="text-[10px] font-bold text-slate-400 mb-2 pl-1 uppercase tracking-wider">
              General
            </div>
